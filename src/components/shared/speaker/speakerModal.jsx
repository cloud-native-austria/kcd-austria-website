...
const SocialMediaLink = ({ link }) => {
  const renderIcon = (type) => {
    ...
    if (link.linkType == 'LinkedIn') return (
    ...
    if (link.linkType == 'Sessionize') return (
    ...
    if (link.linkType == 'Twitter') return (
      <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 512 512" className="fill-black">
...
SocialMediaLink.propTypes = {
  ...
  linkType: PropTypes.oneOf(['Twitter', 'LinkedIn', 'Facebook', 'Instagram', 'Blog', 'Company_Website', 'Sessionize', 'Other']),
};
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center" onClick={handleOverlayClick}>
      ...
      >
        ...
        <div className="flex gap-4 items-center pb-3">
          ...
          <h2 className="text-2xl flex flex-col gap-2 items-start">{speaker.fullName}
            <span className="flex gap-2">{speaker.links?.map((link) => <SocialMediaLink key={link.url} link={link} />)}</span>
          </h2>
        </div>
